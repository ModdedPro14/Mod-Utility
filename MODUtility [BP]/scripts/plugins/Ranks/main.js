...
world.events.beforeChat.subscribe((data) => {
  ...
  if (data.sender.hasTag('staffChat')) {
    for (const player of world.getPlayers({ tags: ['staffChat'] })) {
      player.tell(`§8(§cStaff Chat§8) §r§8[§r${ranks}§r§8] §r${nickname ?? data.sender.name}§r§7 ${new Date().toLocaleTimeString().replace('-', 1)}§l§6 >>§r ${message = Object.keys(emj).forEach(key => message = message.replaceAll(key, emj[key])) ?? message}`);
    }
  } else {
