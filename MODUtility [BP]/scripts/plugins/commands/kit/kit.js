...
server.commands.register({
    ...
    if (create.includes(args[0])) {
        if (!player.hasTag(adminTag)) return player.tell(`§cUnknown command: ${message.replace(main.prefix, '')}. Please check that the command exists and that you have permission to use it.`)
        ...
        if (!server.kit.has(args[1])) return player.tell('§cThat kit doesn\'t exist');
        server.kit.delete(args[1]);
        player.tell(`§cYou have removed the kit ${args[1]}§r §csuccessfuly`)
    } else if (list.includes(args[0])) {
        player.tell(`§c----------------\nAvailable kits:${server.kit.allKeys()}\n§c----------------`)
    } else if (set.includes(args[0])) {
        if (!player.hasTag(adminTag)) return player.tell(`§cUnknown command: ${message.replace(main.prefix, '')}. Please check that the command exists and that you have permission to use it.`)
        let permission;
        let price;
        if (!args[1]) return player.tell('§cYou must provide a kit name');
        if (!server.kit.has(args[1])) return player.tell('§cThat kit dosent exist');
        ...
        for (let i = 0; i < inventory.size; i++) {
            ...
            if (!item) continue; 
            itemCount++;
            items.push(server.utils.getItemData(item));
        }
        if (!items.length)
        ...
        server.kit.write(args[1], data);
        }
    } else if (buy.includes(args[0])) {
        if (!args[1]) return player.tell('§cYou must provide a kit name')
        if (!server.kit.has(args[1])) return player.tell('§cYou must provide an existing kit name')
        const Data = server.kit.read(args[1])
        ...
        if (server.scoreboard.get(player, "money") < Data.price) return player.tell(`§cYou need ${Data.price - server.scoreboard.get(player, "money")} more to buy this item`)
        const items = Data.items;
        if (!items) return player.tell('§cThere are no items set in this kit')
        for (const item of items) {
            inventory.addItem(server.utils.newItem(item));
        }
