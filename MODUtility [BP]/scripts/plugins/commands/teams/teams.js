...
server.commands.register({
    ...
    if (args[0] == 'create') {
        ...
        if (server.teams.isTeamOwner(player)) return player.tell('§cYou already have created a team')
        if (server.teams.isInTeam(player)) return player.tell('§cYou cant create a team when your in a team')
        if (server.teams.database.has(args[1])) return player.tell('§cThis team already exists')
        if (args[2]) return player.tell('§cYou cant have spaces in your teams name')
        server.teams.newTeam(args[1], player.name)
        ...
        player.tell(`§c----------------\nAvailable Teams:${server.teams.database.allKeys()}\n§c----------------`)
    } else if (args[0] == 'view') {
        if (!args[1]) return player.tell('§cYou must provide a teams name')
        if (!server.teams.database.has(args[1])) return player.tell('§cYou must provide a teams name that exist')
        const Data = server.teams.database.read(args[1])
        let text = '\n'
        ...
        if (server.teams.isTeamOwner(player)) return player.tell('§cYou cant leave your own team')
        if (!server.teams.isInTeam(player)) return player.tell('§cYou arent in a team')
        player.tell(`§cYou have left the team ${server.teams.getPlayersTeam(player)}`)
        player.removeTag(`rank:${server.teams.getPlayersTeam(player)}`)
        player.removeTag(`team:${server.teams.getPlayersTeam(player)}`)
    } else if (args[0] == 'delete') {
        if (!server.teams.isInTeam(player)) return player.tell('§cYou arent in a team')
        if (!server.teams.isTeamOwner(player)) return player.tell('§cYou arent the owner of the team to delete this team')
        player.tell(`§cYou have deleted the team ${server.teams.getPlayersTeam(player)}`)
        server.teams.database.delete(server.teams.getPlayersOwnerTeam(player))
        player.removeTag(`rank:${server.teams.getPlayersTeam(player)} §r§7- §eLeader`)
        player.removeTag(`teamOwner:${server.teams.getPlayersOwnerTeam(player)}`)
        player.removeTag(`team:${server.teams.getPlayersTeam(player)}`)
    } else if (args[0] == 'invite') {
        const foundPlayer = server.commands.addUserOption(player, args[1])
        if (!foundPlayer) return;
        if (!server.teams.isInTeam(player)) return player.tell('§cYou arent in a team to invite any one')
        if (foundPlayer == player) return player.tell("§cYou can't send a invite request to yourself")
        if (server.teams.isInTeam(foundPlayer)) return player.tell(`${foundPlayer.name} §r§care in a team so u cant invite them`)
        if (server.teams.getPlayersTeam(foundPlayer) == server.teams.getPlayersTeam(player)) return player.tell(`${foundPlayer.name} §r§care already in your team`)
        ...
        player.addTag(`team:${server.teams.getPlayersTeam(requester)}`)
        player.addTag(`rank:${server.teams.getPlayersTeam(requester)}`)
        ...
        if (!server.teams.isTeamOwner(player)) return player.tell('§cYou arent the team owner to kick anyone')
        const foundPlayer = server.commands.addUserOption(player, args[1])
        if (!foundPlayer) return;
        ...
        foundPlayer.removeTag(`rank:${server.teams.getPlayersTeam(player)}`)
        foundPlayer.removeTag(`team:${server.teams.getPlayersTeam(player)}`)
